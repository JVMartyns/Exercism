...
def approx_average_is_average(hand: list[int]):
    ...
    return card_average(hand) in [approx_average, middle_card]
...
def average_even_is_average_odd(hand: list[int]):
    ...
    """
    even_indexes = [card for card in hand if hand.index(card) % 2 == 0]
    odd_indexes = [card for card in hand if hand.index(card) % 2 != 0]

    even_average = card_average(even_indexes)
    odd_average = card_average(odd_indexes)
